...
def salt_and_pepper(input,prob):
    noise_tensor=torch.rand(input.shape[0], input.shape[1])
...
class IganTrainer:
    ...
    def _critic_train_iteration(self, x1, x2):
        ...
        d_loss = d_generated.mean() - 0.2 * d_real.mean() + 1 * gradient_penalty - 0.8 * d_stable.mean()
    ...
    def train(self, data_loader, epochs, val_images=None, save_training_gif=True):
        if self.tracking_images is None and self.num_tracking_images > 0:
            ...
            # self.tracking_z = (torch.randn(self.num_tracking_images, self.g.z_dim)
            # + salt_and_pepper(torch.randn(self.num_tracking_images, self.g.z_dim),1)).to(self.device)
            self.tracking_z = torch.randn(self.num_tracking_images, self.g.z_dim).to(self.device)
