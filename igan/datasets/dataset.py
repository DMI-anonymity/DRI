...
def our_dagan_dataloader(raw_data, num_classes, transform, batch_size):
    train_x1 = []
    train_x2 = []
    ...
    for i in range(num_classes):
        ...
        train_x1.extend(raw_data[i])
        train_x2.extend(x2_data)
        # train_x2.extend(raw_data[i])

    train_dataset = DaganDataset(train_x1, train_x2, transform)
    return DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=5)
